...
describe('Role API', () => {
    ...
    describe('Authorization Middleware', () => {
        it('should allow Super Admin to access admin routes', async () => {
            ...
            mockDb.query.mockResolvedValueOnce({ rows: [mockSuperAdmin] });
            mockDb.query.mockResolvedValueOnce({ rows: [] }); // default project
        ...
        it('should deny Viewer access to admin-only routes', async () => {
            ...
            // Mock auth middleware queries
            mockDb.query.mockResolvedValueOnce({ rows: [mockViewer] });
            mockDb.query.mockResolvedValueOnce({ rows: [] });
            const res = await (0, supertest_1.default)(app)
    ...
    describe('POST /api/v1/roles', () => {
        ...
        it('should reject duplicate role name', async () => {
            ...
            mockDb.query.mockResolvedValueOnce({ rows: [mockSuperAdmin] });
            mockDb.query.mockResolvedValueOnce({ rows: [] });
            mockDb.query.mockResolvedValueOnce({ rows: [{ id: 1 }] }); // existing role
            const res = await (0, supertest_1.default)(app)
                .post('/api/v1/roles')
                .set('Authorization', `Bearer ${token}`)
    ...
    describe('PUT /api/v1/roles/:id', () => {
        it('should update role successfully', async () => {
            ...
            mockDb.query.mockResolvedValueOnce({ rows: [mockSuperAdmin] });
            mockDb.query.mockResolvedValueOnce({ rows: [] });
            mockDb.query.mockResolvedValueOnce({ rows: [{ id: 5 }] }); // role exists
            mockDb.query.mockResolvedValueOnce({ rows: [] }); // no duplicate name
        ...
        it('should return 404 for non-existent role', async () => {
            ...
            mockDb.query.mockResolvedValueOnce({ rows: [mockSuperAdmin] });
            mockDb.query.mockResolvedValueOnce({ rows: [] });
            mockDb.query.mockResolvedValueOnce({ rows: [] }); // role not found
            const res = await (0, supertest_1.default)(app)
    ...
    describe('DELETE /api/v1/roles/:id', () => {
        ...
        it('should prevent deleting role with assigned users', async () => {
            ...
            mockDb.query.mockResolvedValueOnce({ rows: [mockSuperAdmin] });
            mockDb.query.mockResolvedValueOnce({ rows: [] });
            mockDb.query.mockResolvedValueOnce({ rows: [{ count: '5' }] }); // has users
            const res = await (0, supertest_1.default)(app)
