...
describe('CAPA API', () => {
    ...
    describe('GET /api/v1/capa', () => {
        ...
        it('should require authentication', async () => {
            const res = await (0, supertest_1.default)(app).get('/api/v1/capa');
            expect(res.status).toBe(401);
        });
    });
    describe('GET /api/v1/capa/analytics', () => {
        it('should require authentication', async () => {
            const res = await (0, supertest_1.default)(app).get('/api/v1/capa/analytics');
            expect(res.status).toBe(401);
        });
    });
    ...
    describe('PUT /api/v1/capa/:id', () => {
        it('should update CAPA', async () => {
            ...
            mockDb.query.mockResolvedValueOnce({ rows: [mockPackageManager] });
            mockDb.query.mockResolvedValueOnce({ rows: [] });
            mockDb.query.mockResolvedValueOnce({ rows: [sampleCAPA] });
            mockDb.query.mockResolvedValueOnce({ rows: [] });
            const res = await (0, supertest_1.default)(app)
                .put('/api/v1/capa/1')
                .set('Authorization', `Bearer ${token}`)
...
describe('CAPA Business Logic', () => {
    describe('CAPA Number Format', () => {
        const generateCAPANumber = (year, count) => `CAPA-${year}-${String(count).padStart(4, '0')}`;
...
//# sourceMappingURL=capa.test.js.map