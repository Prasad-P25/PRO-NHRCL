    ...
    constructor() {
        this.getAllCategories = async (req, res, next) => {
            try {
                ...
                const categoryResult = await connection_1.db.query(`SELECT ac.*,
         ...
         ORDER BY ac.display_order`);
                ...
                const categoriesWithSections = await Promise.all(categoryResult.rows.map(async (cat) => {
                    const sectionsResult = await connection_1.db.query(`SELECT s.*,
                    json_agg(
                      json_build_object(
                        'id', ai.id,
                        'srNo', ai.sr_no,
             ...
             GROUP BY s.id
             ORDER BY s.display_order`, [cat.id]);
                    return {
                        id: cat.id,
                        code: cat.code,
                        name: cat.name,
        ...
        this.getCategoryById = async (req, res, next) => {
            try {
                const { id } = req.params;
                const categoryResult = await connection_1.db.query('SELECT * FROM audit_categories WHERE id = $1', [id]);
        ...
        this.getCategoryItems = async (req, res, next) => {
            try {
                const { id } = req.params;
                const result = await connection_1.db.query(`SELECT ai.*, s.code as section_code, s.name as section_name
         FROM audit_items ai
         JOIN audit_sections s ON ai.section_id = s.id
         WHERE s.category_id = $1 AND ai.is_active = true
         ORDER BY s.display_order, ai.sr_no`, [id]);
                res.json({
                    success: true,
        ...
        this.createCategory = async (req, res, next) => {
            try {
                ...
                const result = await connection_1.db.query(`INSERT INTO audit_categories (code, name, full_title, description, type, applicable_standards, display_order)
         VALUES ($1, $2, $3, $4, $5, $6, $7)
         RETURNING *`, [code, name, fullTitle || null, description || null, type || null, applicableStandards || null, displayOrder || 999]);
                res.status(201).json({
                    success: true,
                    data: result.rows[0],
        ...
        this.updateCategory = async (req, res, next) => {
            try {
                ...
                await connection_1.db.query(`UPDATE audit_categories SET
         ...
         WHERE id = $9`, [code, name, fullTitle, description, type, applicableStandards, displayOrder, isActive, id]);
                res.json({
                    success: true,
        ...
        // Create section
        this.createSection = async (req, res, next) => {
            try {
                const { categoryId, code, name, displayOrder } = req.body;
                const result = await connection_1.db.query(`INSERT INTO audit_sections (category_id, code, name, display_order)
         VALUES ($1, $2, $3, $4)
         RETURNING *`, [categoryId, code, name, displayOrder || 999]);
                res.status(201).json({
                    success: true,
                    data: result.rows[0],
        ...
        // Update section
        this.updateSection = async (req, res, next) => {
            try {
                const { id } = req.params;
                const { code, name, displayOrder } = req.body;
                await connection_1.db.query(`UPDATE audit_sections SET
         code = COALESCE($1, code),
         name = COALESCE($2, name),
         display_order = COALESCE($3, display_order)
         WHERE id = $4`, [code, name, displayOrder, id]);
                res.json({
                    success: true,
        ...
        // Delete section
        this.deleteSection = async (req, res, next) => {
            try {
                const { id } = req.params;
                // Check if section has items
                const itemCheck = await connection_1.db.query('SELECT COUNT(*) FROM audit_items WHERE section_id = $1', [id]);
                if (parseInt(itemCheck.rows[0].count) > 0) {
                    throw new errorHandler_1.AppError('Cannot delete section with audit items', 400);
                }
                await connection_1.db.query('DELETE FROM audit_sections WHERE id = $1', [id]);
                res.json({
                    success: true,
        ...
        this.createItem = async (req, res, next) => {
            try {
                ...
                const result = await connection_1.db.query(`INSERT INTO audit_items (section_id, sr_no, audit_point, standard_reference, evidence_required, priority)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING *`, [sectionId, srNo, auditPoint, standardReference || null, evidenceRequired || null, priority || 'P1']);
                res.status(201).json({
                    success: true,
                    data: result.rows[0],
        ...
        this.updateItem = async (req, res, next) => {
            try {
                ...
                await connection_1.db.query(`UPDATE audit_items SET
         ...
         WHERE id = $7`, [srNo, auditPoint, standardReference, evidenceRequired, priority, isActive, id]);
                res.json({
                    success: true,
        ...
        this.deleteItem = async (req, res, next) => {
            try {
                const { id } = req.params;
                await connection_1.db.query('UPDATE audit_items SET is_active = false WHERE id = $1', [id]);
                res.json({
                    success: true,
