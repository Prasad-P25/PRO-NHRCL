...
class DashboardController {
    constructor() {
        this.getOverview = async (req, res, next) => {
            try {
                ...
                const projectFilterDirect = projectId
                    ...
                    : 'WHERE';
                // Overall compliance
                const complianceResult = await connection_1.db.query(`SELECT
         ...
         ${projectId ? `AND p.project_id = ${projectId}` : ''}`);
                ...
                const prevComplianceResult = await connection_1.db.query(`SELECT COALESCE(AVG(a.compliance_percentage), 0) as avg_compliance
         FROM audits a
         ${projectId ? 'JOIN packages p ON a.package_id = p.id' : ''}
         WHERE a.status IN ('Approved', 'Closed')
         ...
         ${projectId ? `AND p.project_id = ${projectId}` : ''}`);
                // Open NCs
                const ncResult = await connection_1.db.query(`SELECT COUNT(*) as open_ncs
         ...
         ${projectId ? `AND p.project_id = ${projectId}` : ''}`);
                // Previous month NCs
                const prevNcResult = await connection_1.db.query(`SELECT COUNT(*) as open_ncs
         FROM audit_responses ar
         JOIN audits a ON ar.audit_id = a.id
         ${projectId ? 'JOIN packages p ON a.package_id = p.id' : ''}
         WHERE ar.status = 'NC'
         ...
         ${projectId ? `AND p.project_id = ${projectId}` : ''}`);
                ...
                const capaStatusResult = await connection_1.db.query(`SELECT c.status, COUNT(*) as count
         FROM capa c
         ${projectId ? `
         ...
         GROUP BY c.status`);
                // CAPA overdue
                const capaOverdueResult = await connection_1.db.query(`SELECT COUNT(*) as overdue
         FROM capa c
         ${projectId ? `
         JOIN audit_responses ar ON c.response_id = ar.id
         JOIN audits a ON ar.audit_id = a.id
         ...
         AND c.target_date < CURRENT_DATE`);
                ...
                const ltiResult = await connection_1.db.query(`SELECT COALESCE(MAX(ke.actual_value), 0) as days_without_lti
         FROM kpi_entries ke
         ...
         ${projectId ? `AND p.project_id = ${projectId}` : ''}`);
                ...
                const packageComplianceResult = await connection_1.db.query(`SELECT p.id, p.code, p.name,
         ...
         ORDER BY p.code`);
                ...
                const trendResult = await connection_1.db.query(`SELECT
         ...
         ORDER BY DATE_TRUNC('month', a.created_at)`);
                ...
                const ncByCategoryResult = await connection_1.db.query(`SELECT c.code, c.name, COUNT(*) as count
         FROM audit_responses ar
         ...
         LIMIT 10`);
                // Recent audits
                const recentAuditsResult = await connection_1.db.query(`SELECT a.*, p.code as package_code, p.name as package_name, u.name as auditor_name
         FROM audits a
         JOIN packages p ON a.package_id = p.id
         ...
         LIMIT 5`);
                ...
                const recentActivityResult = await connection_1.db.query(`(SELECT 'audit' as type, a.audit_number as reference, a.status,
         ...
         LIMIT 10`);
                ...
                const auditStatusResult = await connection_1.db.query(`SELECT a.status, COUNT(*) as count
         FROM audits a
         ${projectId ? 'JOIN packages p ON a.package_id = p.id' : ''}
         ${projectId ? `WHERE p.project_id = ${projectId}` : ''}
         GROUP BY a.status`);
                ...
                const capaStatus = { Open: 0, 'In Progress': 0, Closed: 0 };
        ...
        this.getPackageDashboard = async (req, res, next) => {
            try {
                const { id } = req.params;
         ...
         FROM audits
         WHERE package_id = $1
         AND status IN ('Approved', 'Closed')`, [id]);
                res.json({
                    success: true,
                    data: {
                        stats: {
        ...
        this.getProjectComparison = async (req, res, next) => {
            try {
                // Get all projects with their key metrics
                const projectMetrics = await connection_1.db.query(`
        SELECT
          pr.id,
          pr.code,
          pr.name,
      ...
      `);
                ...
                const complianceTrend = await connection_1.db.query(`
        SELECT
          pr.id as project_id,
      ...
      `);
                ...
                const ncByProject = await connection_1.db.query(`
        SELECT
          pr.code as project_code,
      ...
      `);
        ...
        this.getKPISummary = async (req, res, next) => {
            try {
      ...
      `;
                ...
                const leadingResult = await connection_1.db.query(leadingQuery, params);
      ...
      `;
                const laggingResult = await connection_1.db.query(laggingQuery, [currentMonth, currentYear]);
                res.json({
                    success: true,
                    data: {
