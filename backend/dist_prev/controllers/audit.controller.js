...
class AuditController {
    constructor() {
        this.getAllAudits = async (req, res, next) => {
            try {
      ...
      `;
                ...
                const countResult = await connection_1.db.query(countQuery, countParams);
        ...
        this.createAudit = async (req, res, next) => {
            try {
                ...
                // Get package code
                const packageResult = await connection_1.db.query('SELECT code FROM packages WHERE id = $1', [packageId]);
                if (packageResult.rows.length === 0) {
                    throw new errorHandler_1.AppError('Package not found', 404);
                }
                // Generate audit number
                const year = new Date().getFullYear();
                const countResult = await connection_1.db.query('SELECT COUNT(*) FROM audits WHERE package_id = $1 AND EXTRACT(YEAR FROM created_at) = $2', [packageId, year]);
                ...
                const itemCountResult = await connection_1.db.query(`SELECT COUNT(*) FROM audit_items ai
         JOIN audit_sections s ON ai.section_id = s.id
         WHERE s.category_id = ANY($1) AND ai.is_active = true`, [categoryIds]);
                const totalItems = parseInt(itemCountResult.rows[0].count);
                // Create audit
                const result = await connection_1.db.query(`INSERT INTO audits (audit_number, package_id, audit_type, auditor_id, scheduled_date, contractor_rep, total_items, status)
         VALUES ($1, $2, $3, $4, $5, $6, $7, 'Draft')
         RETURNING *`, [auditNumber, packageId, auditType, req.user.id, scheduledDate || null, contractorRep || null, totalItems]);
                const audit = result.rows[0];
                // Link categories to audit
                for (const categoryId of categoryIds) {
                    await connection_1.db.query('INSERT INTO audit_category_selection (audit_id, category_id) VALUES ($1, $2)', [audit.id, categoryId]);
                }
                res.status(201).json({
                    success: true,
                    data: {
        ...
        this.getAuditById = async (req, res, next) => {
            try {
                const { id } = req.params;
                const result = await connection_1.db.query(`SELECT a.*, p.code as package_code, p.name as package_name,
                u.name as auditor_name, u.email as auditor_email,
         ...
         WHERE a.id = $1`, [id]);
                ...
                const categoriesResult = await connection_1.db.query(`SELECT ac.*, acs.id as selection_id
         FROM audit_categories ac
         JOIN audit_category_selection acs ON ac.id = acs.category_id
         WHERE acs.audit_id = $1
         ORDER BY ac.display_order`, [id]);
                res.json({
                    success: true,
                    data: {
                        id: audit.id,
        ...
        this.deleteAudit = async (req, res, next) => {
            try {
                const { id } = req.params;
                // Check if audit is in Draft status
                const auditResult = await connection_1.db.query('SELECT status FROM audits WHERE id = $1', [id]);
                if (auditResult.rows.length === 0) {
                    throw new errorHandler_1.AppError('Audit not found', 404);
                }
                if (auditResult.rows[0].status !== 'Draft') {
                    throw new errorHandler_1.AppError('Can only delete draft audits', 400);
                }
                await connection_1.db.query('DELETE FROM audit_category_selection WHERE audit_id = $1', [id]);
                await connection_1.db.query('DELETE FROM audits WHERE id = $1', [id]);
                res.json({
                    success: true,
        ...
        this.submitAudit = async (req, res, next) => {
            try {
                const { id } = req.params;
                // Get audit details before updating
                const auditResult = await connection_1.db.query(`SELECT a.audit_number, a.package_id, p.code as package_code
         FROM audits a
         JOIN packages p ON a.package_id = p.id
         WHERE a.id = $1`, [id]);
                // Check if all NC items have at least one evidence
                const ncWithoutEvidence = await connection_1.db.query(`SELECT ar.id, ai.sr_no, ai.audit_point, s.name as section_name
         ...
         HAVING COUNT(ae.id) = 0`, [id]);
                // Evidence requirement for non-compliant items
                if (ncWithoutEvidence.rows.length > 0) {
                    const missingItems = ncWithoutEvidence.rows.map((r) => `${r.section_name} - Item ${r.sr_no}`);
                    throw new errorHandler_1.AppError(`Cannot submit audit. The following NC items require evidence: ${missingItems.join(', ')}`, 400);
                }
                await connection_1.db.query(`UPDATE audits SET status = 'Pending Review', completed_at = CURRENT_TIMESTAMP WHERE id = $1`, [id]);
                // Auto-create CAPAs for items marked as "CAPA Required"
                // Debug: Check all responses for this audit
                const allResponses = await connection_1.db.query(`SELECT ar.id, ar.status, ar.capa_required, ar.observation
         FROM audit_responses ar WHERE ar.audit_id = $1`, [id]);
                logger_1.logger.info(`Audit ${id}: Found ${allResponses.rows.length} total responses`);
                // Log each response for debugging
                allResponses.rows.forEach((r, idx) => {
                    logger_1.logger.info(`Audit ${id} Response ${idx}: status=${r.status}, capa_required=${r.capa_required} (type: ${typeof r.capa_required})`);
                });
                // Handle both boolean true and string 't' from PostgreSQL
                const ncWithCapa = allResponses.rows.filter((r) => r.status === 'NC' && (r.capa_required === true || r.capa_required === 't'));
                logger_1.logger.info(`Audit ${id}: Found ${ncWithCapa.length} NC responses with capa_required=true`);
                const capaRequiredResponses = await connection_1.db.query(`SELECT ar.id as response_id, ar.observation, ai.audit_point, a.audit_number
         FROM audit_responses ar
         JOIN audit_items ai ON ar.audit_item_id = ai.id
         JOIN audits a ON ar.audit_id = a.id
         WHERE ar.audit_id = $1 AND ar.capa_required = true AND ar.status = 'NC'
         AND NOT EXISTS (SELECT 1 FROM capa c WHERE c.response_id = ar.id)`, [id]);
                logger_1.logger.info(`Audit ${id}: Query returned ${capaRequiredResponses.rows.length} eligible for CAPA creation`);
                let capasCreated = 0;
                const year = new Date().getFullYear();
                // Get current count once for all CAPAs to avoid race conditions
                const countResult = await connection_1.db.query(`SELECT COUNT(*) FROM capa WHERE capa_number LIKE $1`, [`CAPA-${year}-%`]);
                ...
                if (capaRequiredResponses.rows.length > 0) {
                    ...
                    if (capaParams.length > 0) {
                        await connection_1.db.query(`INSERT INTO capa (capa_number, response_id, finding_description, status)
             VALUES ${capaParams.join(', ')}`, capaValues);
                ...
                if (auditResult.rows.length > 0) {
                    ...
                    const [pmcHeads, managers] = await Promise.all([
                        (0, notification_controller_1.getUsersByRole)('PMC Head'),
                        (0, notification_controller_1.getPackageManagers)(package_id)
                    ]);
                    // Create all notifications in parallel (non-blocking)
                    const pmcNotifications = pmcHeads.map(userId => (0, notification_controller_1.createNotification)(userId, 'audit_submitted', 'Audit Submitted for Review', `Audit ${audit_number} (Package ${package_code}) has been submitted for review`, {
                        fromUserId: req.user?.id,
                        entityType: 'audit',
                        entityId: parseInt(id),
                        actionUrl: `/audits/${id}`,
                    }).catch(err => logger_1.logger.error('Failed to create PMC notification:', err)));
                    const managerNotifications = managers.map(managerId => (0, notification_controller_1.createNotification)(managerId, 'audit_submitted', 'Audit Submitted for Review', `Audit ${audit_number} has been submitted and is pending your review`, {
                        fromUserId: req.user?.id,
                        entityType: 'audit',
                        entityId: parseInt(id),
                        actionUrl: `/audits/${id}`,
                    }).catch(err => logger_1.logger.error('Failed to create manager notification:', err)));
        ...
        this.approveAudit = async (req, res, next) => {
            try {
                const { id } = req.params;
                const { comments } = req.body;
                // Get audit details
                const auditResult = await connection_1.db.query(`SELECT a.audit_number, a.auditor_id, p.code as package_code
         FROM audits a
         JOIN packages p ON a.package_id = p.id
         WHERE a.id = $1`, [id]);
         ...
         WHERE id = $2`, [req.user.id, id]);
                logger_1.logger.info(`Audit ${id} approved and locked by user ${req.user.id}`);
                // Notify the auditor
                if (auditResult.rows.length > 0) {
                    const { audit_number, auditor_id, package_code } = auditResult.rows[0];
                    if (auditor_id && auditor_id !== req.user.id) {
                        await (0, notification_controller_1.createNotification)(auditor_id, 'audit_approved', 'Audit Approved', `Your audit ${audit_number} (Package ${package_code}) has been approved`, {
                            fromUserId: req.user?.id,
                            entityType: 'audit',
                            entityId: parseInt(id),
                            actionUrl: `/audits/${id}`,
        ...
        this.rejectAudit = async (req, res, next) => {
            try {
                const { id } = req.params;
         ...
         JOIN packages p ON a.package_id = p.id
         WHERE a.id = $1`, [id]);
                await connection_1.db.query(`UPDATE audits SET status = 'Rejected' WHERE id = $1`, [id]);
                // Notify the auditor
                if (auditResult.rows.length > 0) {
                    const { audit_number, auditor_id, package_code } = auditResult.rows[0];
                    if (auditor_id && auditor_id !== req.user.id) {
                        await (0, notification_controller_1.createNotification)(auditor_id, 'audit_rejected', 'Audit Returned for Revision', `Your audit ${audit_number} (Package ${package_code}) requires revision${reason ? `: ${reason}` : ''}`, {
                            fromUserId: req.user?.id,
                            entityType: 'audit',
                            entityId: parseInt(id),
                            actionUrl: `/audits/${id}`,
        ...
        this.getAuditResponses = async (req, res, next) => {
            try {
                const { id } = req.params;
                const result = await connection_1.db.query(`SELECT ar.*, ai.sr_no, ai.audit_point, ai.standard_reference, ai.priority,
         ...
         ORDER BY c.display_order, s.display_order, ai.sr_no`, [id]);
                res.json({
                    success: true,
        ...
        this.saveAuditResponses = async (req, res, next) => {
            try {
                const { id } = req.params;
                const { responses } = req.body;
                // Check if audit is locked (approved)
                const auditCheck = await connection_1.db.query('SELECT status, locked_at FROM audits WHERE id = $1', [id]);
                if (auditCheck.rows.length === 0) {
                    throw new errorHandler_1.AppError('Audit not found', 404);
                }
                if (auditCheck.rows[0].status === 'Approved' || auditCheck.rows[0].locked_at) {
                    throw new errorHandler_1.AppError('This audit has been approved and is locked. No changes allowed.', 403);
                }
                let savedCount = 0;
                ...
                for (const response of responses) {
                    ...
                    const existingResponse = await connection_1.db.query('SELECT * FROM audit_responses WHERE audit_id = $1 AND audit_item_id = $2', [id, response.auditItemId]);
                    const oldData = existingResponse.rows[0] || null;
                    const isUpdate = !!oldData;
                    // Insert or update response
                    const result = await connection_1.db.query(`INSERT INTO audit_responses (audit_id, audit_item_id, status, observation, risk_rating, capa_required, remarks, updated_by)
           ...
           RETURNING id`, [
                    ...
                    await connection_1.db.query(`INSERT INTO audit_response_history
         ...
         WHERE id = $5`, [compliantCount, nonCompliantCount, naCount, compliancePercentage, id]);
                res.json({
                    success: true,
                    data: {
                        savedCount,
        ...
        this.uploadEvidence = async (req, res, next) => {
            try {
                ...
                const auditCheck = await connection_1.db.query(`SELECT a.status, a.locked_at FROM audits a
         JOIN audit_responses ar ON ar.audit_id = a.id
         WHERE ar.id = $1`, [responseId]);
                if (auditCheck.rows.length > 0 &&
                    (auditCheck.rows[0].status === 'Approved' || auditCheck.rows[0].locked_at)) {
                    throw new errorHandler_1.AppError('This audit has been approved and is locked. No changes allowed.', 403);
                }
                const result = await connection_1.db.query(`INSERT INTO audit_evidences (response_id, file_name, file_path, file_type, file_size, uploaded_by)
         VALUES ($1, $2, $3, $4, $5, $6)
        ...
        this.deleteEvidence = async (req, res, next) => {
            try {
         ...
         WHERE ar.id = $1`, [responseId]);
                ...
                await connection_1.db.query('DELETE FROM audit_evidences WHERE id = $1 AND response_id = $2', [evidenceId, responseId]);
                res.json({
                    success: true,
        ...
        this.exportToWord = async (req, res, next) => {
            try {
                const { id } = req.params;
                // Get audit details
                const auditResult = await connection_1.db.query(`SELECT a.*, p.code as package_code, p.name as package_name,
         ...
         WHERE a.id = $1`, [id]);
                ...
                const ncResponses = await connection_1.db.query(`SELECT ar.*, ai.sr_no, ai.audit_point, ai.standard_reference, ai.priority,
                s.code as section_code, s.name as section_name,
                c.code as category_code, c.name as category_name,
                COALESCE(
                  json_agg(
         ...
         ORDER BY c.display_order, s.display_order, ai.sr_no`, [id]);
                ...
                docChildren.push(new docx_1.Paragraph({
                    children: [
                        new docx_1.TextRun({ text: 'Audit Number: ', bold: true }),
                        new docx_1.TextRun({ text: audit.audit_number }),
                    ],
                    spacing: { after: 100 },
                }), new docx_1.Paragraph({
                    children: [
                        new docx_1.TextRun({ text: 'Package: ', bold: true }),
                        new docx_1.TextRun({ text: `${audit.package_code} - ${audit.package_name}` }),
                    ],
                    spacing: { after: 100 },
                }), new docx_1.Paragraph({
                    children: [
                        new docx_1.TextRun({ text: 'Audit Date: ', bold: true }),
                        new docx_1.TextRun({ text: audit.audit_date ? new Date(audit.audit_date).toLocaleDateString() : 'N/A' }),
                    ],
                    spacing: { after: 100 },
                }), new docx_1.Paragraph({
                    children: [
                        new docx_1.TextRun({ text: 'Auditor: ', bold: true }),
                        new docx_1.TextRun({ text: audit.auditor_name || 'N/A' }),
                    ],
                    spacing: { after: 300 },
                }));
                ...
                const headerRow = new docx_1.TableRow({
                    ...
                    children: [
                        new docx_1.TableCell({
                            ...
                            children: [new docx_1.Paragraph({ alignment: docx_1.AlignmentType.CENTER, children: [new docx_1.TextRun({ text: 'S.No', bold: true })] })],
                        }),
                        new docx_1.TableCell({
                            width: { size: 25, type: docx_1.WidthType.PERCENTAGE },
                            shading: { fill: 'CCCCCC' },
                            verticalAlign: docx_1.VerticalAlign.CENTER,
                            children: [new docx_1.Paragraph({ alignment: docx_1.AlignmentType.CENTER, children: [new docx_1.TextRun({ text: 'Evidence Photo', bold: true })] })],
                        }),
                        new docx_1.TableCell({
                            width: { size: 10, type: docx_1.WidthType.PERCENTAGE },
                            shading: { fill: 'CCCCCC' },
                            verticalAlign: docx_1.VerticalAlign.CENTER,
                            children: [new docx_1.Paragraph({ alignment: docx_1.AlignmentType.CENTER, children: [new docx_1.TextRun({ text: 'Risk Level', bold: true })] })],
                        }),
                        new docx_1.TableCell({
                            width: { size: 35, type: docx_1.WidthType.PERCENTAGE },
                            shading: { fill: 'CCCCCC' },
                            verticalAlign: docx_1.VerticalAlign.CENTER,
                            children: [new docx_1.Paragraph({ alignment: docx_1.AlignmentType.CENTER, children: [new docx_1.TextRun({ text: 'Reference & Recommendation', bold: true })] })],
                        }),
                        new docx_1.TableCell({
                            width: { size: 25, type: docx_1.WidthType.PERCENTAGE },
                            shading: { fill: 'CCCCCC' },
                            verticalAlign: docx_1.VerticalAlign.CENTER,
                            children: [new docx_1.Paragraph({ alignment: docx_1.AlignmentType.CENTER, children: [new docx_1.TextRun({ text: 'Remarks', bold: true })] })],
                        }),
                    ],
                });
                // Data rows
                ...
                for (const response of ncResponses.rows) {
                        ...
                        if (photo.filePath && photo.fileType && photo.fileType.startsWith('image/')) {
                            try {
                                ...
                                if (fs.existsSync(fullPath)) {
                                    ...
                                    photoChildren.push(new docx_1.Paragraph({
                                        children: [
                                            new docx_1.ImageRun({
                                                data: imageData,
                                ...
                                logger_1.logger.error(`Failed to load image: ${photo.filePath}`, err);
                            }
                        }
                    }
                    if (photoChildren.length === 0) {
                        photoChildren.push(new docx_1.Paragraph({ children: [new docx_1.TextRun({ text: 'No photo', italics: true })], alignment: docx_1.AlignmentType.CENTER }));
                    }
                    ...
                    dataRows.push(new docx_1.TableRow({
                        children: [
                            ...
                            new docx_1.TableCell({
                                verticalAlign: docx_1.VerticalAlign.CENTER,
                                children: [new docx_1.Paragraph({ text: String(serialNo), alignment: docx_1.AlignmentType.CENTER })],
                            }),
                            // Evidence Photo
                            new docx_1.TableCell({
                                verticalAlign: docx_1.VerticalAlign.CENTER,
                            ...
                            new docx_1.TableCell({
                                ...
                                children: [
                                    new docx_1.Paragraph({ children: [new docx_1.TextRun({ text: reference })] }),
                                    ...(standardRef ? [new docx_1.Paragraph({ children: [new docx_1.TextRun({ text: standardRef, italics: true, size: 20 })] })] : []),
                                ],
                            }),
                            // Remarks
                            new docx_1.TableCell({
                                verticalAlign: docx_1.VerticalAlign.TOP,
                                children: [
                                    new docx_1.Paragraph({ children: [new docx_1.TextRun({ text: response.observation || '' })] }),
                                    ...(response.remarks ? [new docx_1.Paragraph({ spacing: { before: 100 }, children: [new docx_1.TextRun({ text: response.remarks })] })] : []),
                                ],
                            }),
                        ],
                    }));
                ...
                docChildren.push(new docx_1.Paragraph({
                    ...
                    children: [
                        new docx_1.TextRun({ text: 'Total Items: ', bold: true }),
                        new docx_1.TextRun({ text: String(audit.total_items || 0) }),
                    ],
                    spacing: { after: 100 },
                }), new docx_1.Paragraph({
                    children: [
                        new docx_1.TextRun({ text: 'Compliant: ', bold: true }),
                        new docx_1.TextRun({ text: String(audit.compliant_count || 0) }),
                    ],
                    spacing: { after: 100 },
                }), new docx_1.Paragraph({
                    children: [
                        new docx_1.TextRun({ text: 'Non-Compliant: ', bold: true }),
                        new docx_1.TextRun({ text: String(audit.non_compliant_count || 0) }),
                    ],
                    spacing: { after: 100 },
                }), new docx_1.Paragraph({
                    children: [
                        new docx_1.TextRun({ text: 'Compliance Percentage: ', bold: true }),
                        new docx_1.TextRun({ text: `${audit.compliance_percentage || 0}%` }),
                    ],
                }));
                // Create document
        ...
        this.exportNCReport = async (req, res, next) => {
            try {
                const { id } = req.params;
         ...
         WHERE a.id = $1`, [id]);
                ...
                // Get NC responses with evidence
                const ncResponses = await connection_1.db.query(`SELECT ar.*, ai.sr_no, ai.audit_point, ai.standard_reference, ai.evidence_required, ai.priority,
                s.code as section_code, s.name as section_name,
                c.code as category_code, c.name as category_name,
                COALESCE(
                  json_agg(
         ...
         ORDER BY c.display_order, s.display_order, ai.sr_no`, [id]);
                ...
                const loadImageWithDimensions = (filePath) => {
                    try {
                        ...
                        logger_1.logger.warn(`Could not load image: ${filePath}`);
                    }
                    return null;
                };
                // Scale image to fit within max dimensions while preserving aspect ratio
                const scaleImage = (origWidth, origHeight, maxWidth, maxHeight) => {
                ...
                const headerRow = new docx_1.TableRow({
                    ...
                    children: [
                        new docx_1.TableCell({
                            ...
                            shading: { fill: '1F4E79', type: docx_1.ShadingType.CLEAR },
                            verticalAlign: docx_1.VerticalAlign.CENTER,
                            children: [
                                new docx_1.Paragraph({
                                    alignment: docx_1.AlignmentType.CENTER,
                                    children: [new docx_1.TextRun({ text: 'S.No', bold: true, color: 'FFFFFF', size: 22 })],
                                }),
                            ],
                        }),
                        new docx_1.TableCell({
                            width: { size: 22, type: docx_1.WidthType.PERCENTAGE },
                            shading: { fill: '1F4E79', type: docx_1.ShadingType.CLEAR },
                            verticalAlign: docx_1.VerticalAlign.CENTER,
                            children: [
                                new docx_1.Paragraph({
                                    alignment: docx_1.AlignmentType.CENTER,
                                    children: [new docx_1.TextRun({ text: 'EVIDENCE PHOTOS', bold: true, color: 'FFFFFF', size: 22 })],
                                }),
                            ],
                        }),
                        new docx_1.TableCell({
                            ...
                            children: [
                                new docx_1.Paragraph({
                                    alignment: docx_1.AlignmentType.CENTER,
                                    children: [new docx_1.TextRun({ text: 'Risk Level', bold: true, color: 'FFFFFF', size: 22 })],
                                }),
                            ],
                        }),
                        new docx_1.TableCell({
                            width: { size: 32, type: docx_1.WidthType.PERCENTAGE },
                            shading: { fill: '1F4E79', type: docx_1.ShadingType.CLEAR },
                            verticalAlign: docx_1.VerticalAlign.CENTER,
                            children: [
                                new docx_1.Paragraph({
                                    alignment: docx_1.AlignmentType.CENTER,
                                    children: [new docx_1.TextRun({ text: 'Reference & Recommendation', bold: true, color: 'FFFFFF', size: 22 })],
                                }),
                            ],
                        }),
                        new docx_1.TableCell({
                            ...
                            children: [
                                new docx_1.Paragraph({
                                    alignment: docx_1.AlignmentType.CENTER,
                                    children: [new docx_1.TextRun({ text: 'Evidence Photos & Remarks', bold: true, color: 'FFFFFF', size: 22 })],
                                }),
                            ],
                        }),
                    ],
                ...
                for (const response of ncResponses.rows) {
                    ...
                    for (const photo of evidence) {
                        if (photo.filePath && photo.fileType && photo.fileType.startsWith('image/')) {
                            ...
                            if (imageInfo) {
                                ...
                                const photoParagraph = new docx_1.Paragraph({
                                    alignment: docx_1.AlignmentType.CENTER,
                                    spacing: { after: 100 },
                                    children: [
                                        new docx_1.ImageRun({
                    ...
                    if (leftPhotos.length === 0) {
                        leftPhotos.push(new docx_1.Paragraph({
                            alignment: docx_1.AlignmentType.CENTER,
                            children: [new docx_1.TextRun({ text: 'No photo', italics: true, color: '666666' })],
                        }));
                    }
                    if (rightPhotos.length === 0 && photoIndex > 0) {
                        // Only add placeholder if we have some photos but none for right
                        rightPhotos.push(new docx_1.Paragraph({
                            alignment: docx_1.AlignmentType.CENTER,
                            children: [new docx_1.TextRun({ text: '-', color: '666666' })],
                        }));
                    }
                    ...
                    refChildren.push(new docx_1.Paragraph({
                        spacing: { after: 80 },
                        children: [
                            new docx_1.TextRun({ text: `${response.category_name}`, bold: true, size: 20 }),
                        ],
                    }));
                    ...
                    if (response.observation) {
                        refChildren.push(new docx_1.Paragraph({
                            spacing: { after: 80 },
                            children: [new docx_1.TextRun({ text: response.observation, size: 20 })],
                        }));
                    }
                    else if (response.audit_point) {
                        refChildren.push(new docx_1.Paragraph({
                            spacing: { after: 80 },
                            children: [new docx_1.TextRun({ text: response.audit_point, size: 20 })],
                        }));
                    }
                    // Standard reference
                    if (response.standard_reference) {
                        refChildren.push(new docx_1.Paragraph({
                            ...
                            children: [
                                new docx_1.TextRun({ text: 'Reference: ', bold: true, size: 18 }),
                                new docx_1.TextRun({ text: response.standard_reference, italics: true, size: 18 }),
                            ],
                        }));
                    }
                    // Build remarks column content
                    const remarksChildren = [];
                    // Add photos to remarks column
                    remarksChildren.push(...rightPhotos);
                    // Add remarks text if present
                    if (response.remarks) {
                        remarksChildren.push(new docx_1.Paragraph({
                            spacing: { before: 100, after: 60 },
                            children: [
                                new docx_1.TextRun({ text: 'Remarks: ', bold: true, size: 18 }),
                                new docx_1.TextRun({ text: response.remarks, size: 18 }),
                            ],
                        }));
                    }
                    ...
                    // Create data row
                    const dataRow = new docx_1.TableRow({
                        children: [
                            // S.No
                            new docx_1.TableCell({
                                shading: { fill: rowShading, type: docx_1.ShadingType.CLEAR },
                                verticalAlign: docx_1.VerticalAlign.CENTER,
                                children: [
                                    new docx_1.Paragraph({
                                        alignment: docx_1.AlignmentType.CENTER,
                                        children: [new docx_1.TextRun({ text: String(serialNo), bold: true, size: 22 })],
                                    }),
                                ],
                            }),
                            ...
                            new docx_1.TableCell({
                                ...
                                children: [
                                    new docx_1.Paragraph({
                                        alignment: docx_1.AlignmentType.CENTER,
                                        children: [
                                            new docx_1.TextRun({ text: riskLevelText, bold: true, color: riskColor, size: 24 }),
                                        ],
                                    }),
                                ],
                            }),
                ...
                const ncTable = new docx_1.Table({
                    width: { size: 100, type: docx_1.WidthType.PERCENTAGE },
                    rows: tableRows,
                });
                ...
                docChildren.push(new docx_1.Paragraph({
                    ...
                    children: [
                        new docx_1.TextRun({ text: '9. NC Observations', bold: true, size: 32 }),
                    ],
                }));
                ...
                if (ncResponses.rows.length > 0) {
                    ...
                    docChildren.push(new docx_1.Paragraph({
                        ...
                        children: [
                            new docx_1.TextRun({ text: 'No non-compliance observations found for this audit.', italics: true, size: 24 }),
                        ],
                    }));
                }
                // Summary statistics
                docChildren.push(new docx_1.Paragraph({
                    spacing: { before: 400, after: 200 },
                    children: [new docx_1.TextRun({ text: 'Summary', bold: true, size: 28 })],
                }));
                // Count NC1 and NC2
                const nc1Count = ncResponses.rows.filter((r) => r.risk_rating === 'Critical').length;
                const nc2Count = ncResponses.rows.length - nc1Count;
                docChildren.push(new docx_1.Paragraph({
                    spacing: { after: 100 },
                    children: [
                        new docx_1.TextRun({ text: 'Total NC Observations: ', bold: true, size: 22 }),
                        new docx_1.TextRun({ text: String(ncResponses.rows.length), size: 22 }),
                    ],
                }), new docx_1.Paragraph({
                    spacing: { after: 100 },
                    children: [
                        new docx_1.TextRun({ text: 'NC 1 (Critical): ', bold: true, color: 'FF0000', size: 22 }),
                        new docx_1.TextRun({ text: String(nc1Count), size: 22 }),
                    ],
                }), new docx_1.Paragraph({
                    spacing: { after: 100 },
                    children: [
                        new docx_1.TextRun({ text: 'NC 2 (Major/Minor): ', bold: true, color: 'FF8C00', size: 22 }),
                        new docx_1.TextRun({ text: String(nc2Count), size: 22 }),
                    ],
                }));
                ...
                if (fs.existsSync(PROTECTHER_LOGO)) {
                    try {
                        ...
                        headerContent.push(new docx_1.Paragraph({
                            alignment: docx_1.AlignmentType.CENTER,
                            spacing: { after: 100 },
                            children: [
                                new docx_1.ImageRun({
                        ...
                        logger_1.logger.warn('Could not load logo for header');
                    }
                }
                // Title row
                headerContent.push(new docx_1.Paragraph({
                            ...
                            properties: {
                                page: {
                                    margin: {
                                        ...
                                        bottom: (0, docx_1.convertInchesToTwip)(0.75),
                                        left: (0, docx_1.convertInchesToTwip)(0.5),
                                        right: (0, docx_1.convertInchesToTwip)(0.5),
                                    },
                                },
                            },
                            headers: {
                                ...
                                default: new docx_1.Footer({
                                    children: [
                                        new docx_1.Paragraph({
                                            alignment: docx_1.AlignmentType.CENTER,
                                            children: [
                                                new docx_1.TextRun({ text: 'PROTECTHER Safety Consultants | ', size: 18, color: '666666' }),
        ...
        // Get audit comments
        this.getAuditComments = async (req, res, next) => {
            try {
                const { id } = req.params;
                const result = await connection_1.db.query(`SELECT ac.*, u.name as user_name, u.email as user_email
        ...
        // Add audit comment
        this.addAuditComment = async (req, res, next) => {
            try {
                const { id } = req.params;
                ...
                const result = await connection_1.db.query(`INSERT INTO audit_comments (audit_id, user_id, comment, comment_type, is_internal)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING *`, [id, req.user.id, comment.trim(), commentType || 'general', isInternal || false]);
                res.status(201).json({
                    success: true,
                    data: {
                        id: result.rows[0].id,
        ...
        this.deleteAuditComment = async (req, res, next) => {
            try {
                const { id, commentId } = req.params;
                // Only allow deletion by comment author or admin
                const commentResult = await connection_1.db.query('SELECT user_id FROM audit_comments WHERE id = $1 AND audit_id = $2', [commentId, id]);
                ...
                await connection_1.db.query('DELETE FROM audit_comments WHERE id = $1', [commentId]);
                res.json({
                    success: true,
        ...
        this.getAuditAttachments = async (req, res, next) => {
            try {
                const { id } = req.params;
                const result = await connection_1.db.query(`SELECT aa.*, u.name as uploader_name
         FROM audit_attachments aa
         LEFT JOIN users u ON aa.uploaded_by = u.id
         WHERE aa.audit_id = $1
         ORDER BY aa.uploaded_at DESC`, [id]);
                res.json({
                    success: true,
        ...
        this.uploadAuditAttachment = async (req, res, next) => {
            try {
                const { id } = req.params;
                ...
                const result = await connection_1.db.query(`INSERT INTO audit_attachments (audit_id, file_name, file_path, file_type, file_size, description, uploaded_by)
        ...
        this.deleteAuditAttachment = async (req, res, next) => {
            try {
                const { id, attachmentId } = req.params;
                // Check if attachment exists
                const attachmentResult = await connection_1.db.query('SELECT * FROM audit_attachments WHERE id = $1 AND audit_id = $2', [attachmentId, id]);
                ...
                await connection_1.db.query('DELETE FROM audit_attachments WHERE id = $1', [attachmentId]);
                res.json({
                    success: true,
        ...
        this.getAuditHistory = async (req, res, next) => {
            try {
                const { id } = req.params;
