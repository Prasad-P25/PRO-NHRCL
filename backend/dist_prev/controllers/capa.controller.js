...
class CAPAController {
    constructor() {
        this.getAllCAPA = async (req, res, next) => {
            try {
      ...
      `;
                ...
                // Get total count
                const countResult = await connection_1.db.query(query.replace('SELECT c.*, ar.status as response_status, ai.audit_point,', 'SELECT COUNT(*)').replace('a.audit_number, p.code as package_code, p.name as package_name', ''), params);
        ...
        this.createCAPA = async (req, res, next) => {
            try {
                const errors = (0, express_validator_1.validationResult)(req);
                if (!errors.isEmpty()) {
                    return res.status(400).json({ success: false, errors: errors.array() });
                }
                const { responseId, findingDescription, rootCause, correctiveAction, preventiveAction, responsiblePerson, responsibleDept, targetDate, } = req.body;
                // Generate CAPA number
                const countResult = await connection_1.db.query('SELECT COUNT(*) FROM capa WHERE EXTRACT(YEAR FROM created_at) = $1', [new Date().getFullYear()]);
                const count = parseInt(countResult.rows[0].count) + 1;
                const capaNumber = `CAPA-${new Date().getFullYear()}-${String(count).padStart(4, '0')}`;
                const result = await connection_1.db.query(`INSERT INTO capa (capa_number, response_id, finding_description, root_cause, corrective_action,
         ...
         RETURNING *`, [
                ...
                const auditInfo = await connection_1.db.query(`SELECT a.audit_number, a.package_id, p.code as package_code
         ...
         WHERE ar.id = $1`, [responseId]);
                if (auditInfo.rows.length > 0) {
                    ...
                    const managers = await (0, notification_controller_1.getPackageManagersWithEmail)(package_id);
                    // Create all notifications in parallel
                    const notificationPromises = managers.map(manager => (0, notification_controller_1.createNotification)(manager.id, 'capa_assigned', 'New CAPA Created', `CAPA ${capaNumber} has been created for audit ${audit_number} (Package ${package_code})`, {
                    ...
                    const emailPromises = managers.map(manager => email_service_1.emailService.sendCapaCreated(manager.email, {
                        capaNumber,
                        ...
                        dueDate: targetDate ? (0, date_fns_1.format)(new Date(targetDate), 'PPP') : 'Not set',
        ...
        this.getCAPAById = async (req, res, next) => {
            try {
                const { id } = req.params;
                const result = await connection_1.db.query(`SELECT c.*, ar.status as response_status, ai.audit_point, ai.standard_reference,
                a.audit_number, p.code as package_code, p.name as package_name,
         ...
         WHERE c.id = $1`, [id]);
                if (result.rows.length === 0) {
                    throw new errorHandler_1.AppError('CAPA not found', 404);
                }
        ...
        this.updateCAPA = async (req, res, next) => {
            try {
                const { id } = req.params;
                const { rootCause, correctiveAction, preventiveAction, responsiblePerson, responsibleDept, targetDate, status, } = req.body;
                await connection_1.db.query(`UPDATE capa SET
         ...
         WHERE id = $8`, [rootCause, correctiveAction, preventiveAction, responsiblePerson, responsibleDept, targetDate, status, id]);
                res.json({
                    success: true,
      ...
      `);
                // CAPA by package
                const byPackageResult = await connection_1.db.query(`
        SELECT
      ...
      `);
                ...
                const closureTimeResult = await connection_1.db.query(`
        SELECT
      ...
      `);
                // Top overdue CAPAs
                const topOverdueResult = await connection_1.db.query(`
        SELECT
          c.id,
          c.capa_number,
      ...
      `);
                // Build status object
                const statusBreakdown = { Open: 0, 'In Progress': 0, Closed: 0 };
        ...
        this.closeCAPA = async (req, res, next) => {
            try {
                const { id } = req.params;
                const { verificationRemarks } = req.body;
                // Get CAPA details before closing
                const capaResult = await connection_1.db.query(`SELECT c.capa_number, c.finding_description, a.auditor_id, a.package_id
         FROM capa c
         JOIN audit_responses ar ON c.response_id = ar.id
         JOIN audits a ON ar.audit_id = a.id
         WHERE c.id = $1`, [id]);
         ...
         WHERE id = $3`, [req.user.id, verificationRemarks || null, id]);
                ...
                if (capaResult.rows.length > 0) {
                    ...
                    (0, notification_controller_1.getPackageManagersWithEmail)(package_id).then(managers => {
                        const emailPromises = managers.map(manager => email_service_1.emailService.sendCapaCompleted(manager.email, {
                            ...
                            completedDate: (0, date_fns_1.format)(new Date(), 'PPP'),
                    ...
                    if (auditor_id && auditor_id !== req.user.id) {
                        (0, notification_controller_1.createNotification)(auditor_id, 'capa_verified', 'CAPA Verified & Closed', `${capa_number} has been verified and closed`, {
                            fromUserId: req.user?.id,
                            entityType: 'capa',
                        ...
                        (0, notification_controller_1.getUserEmail)(auditor_id).then(auditorInfo => {
                            if (auditorInfo) {
                                email_service_1.emailService.sendCapaCompleted(auditorInfo.email, {
                                    capaNumber: capa_number,
                                    finding: finding_description?.substring(0, 200) + (finding_description?.length > 200 ? '...' : ''),
                                    completedBy: req.user?.name || 'Verifier',
                                    completedDate: (0, date_fns_1.format)(new Date(), 'PPP'),
