...
class KPIController {
    constructor() {
        ...
        this.getEntries = async (req, res, next) => {
            try {
      ...
      `;
        ...
        this.createEntry = async (req, res, next) => {
            try {
                const errors = (0, express_validator_1.validationResult)(req);
                if (!errors.isEmpty()) {
                    return res.status(400).json({ success: false, errors: errors.array() });
                }
                const { packageId, indicatorId, periodMonth, periodYear, targetValue, actualValue, manHoursWorked, incidentsCount, remarks, } = req.body;
                const result = await connection_1.db.query(`INSERT INTO kpi_entries (package_id, indicator_id, period_month, period_year,
        ...
        this.updateEntry = async (req, res, next) => {
            try {
                const { id } = req.params;
                const { targetValue, actualValue, manHoursWorked, incidentsCount, remarks } = req.body;
                await connection_1.db.query(`UPDATE kpi_entries SET
         ...
         WHERE id = $6`, [targetValue, actualValue, manHoursWorked, incidentsCount, remarks, id]);
                res.json({
                    success: true,
