...
class MaturityController {
    constructor() {
        ...
        this.getMaturityModel = async (req, res, next) => {
            try {
                res.json({
                    success: true,
                    data: MATURITY_MODEL,
      ...
      `;
        ...
        this.getById = async (req, res, next) => {
            try {
                const { id } = req.params;
                const assessmentResult = await connection_1.db.query(`SELECT ma.*, p.code as package_code, p.name as package_name,
                u.name as assessor_name
         FROM maturity_assessments ma
         JOIN packages p ON ma.package_id = p.id
         LEFT JOIN users u ON ma.assessor_id = u.id
         WHERE ma.id = $1`, [id]);
                ...
                // Get responses
                const responsesResult = await connection_1.db.query(`SELECT * FROM maturity_responses WHERE assessment_id = $1 ORDER BY id`, [id]);
                res.json({
                    success: true,
                    data: {
                        id: assessment.id,
        ...
        this.create = async (req, res, next) => {
            try {
                ...
                // Create assessment
                const result = await connection_1.db.query(`INSERT INTO maturity_assessments (package_id, assessment_date, assessor_id, status)
         VALUES ($1, $2, $3, 'Draft')
         RETURNING id`, [packageId, assessmentDate || new Date(), assessorId]);
                ...
                for (const dimension of MATURITY_MODEL.dimensions) {
                    for (const q of dimension.questions) {
                        await connection_1.db.query(`INSERT INTO maturity_responses (assessment_id, dimension, question)
             VALUES ($1, $2, $3)`, [assessmentId, dimension.name, q.question]);
                    }
                }
                res.status(201).json({
                    success: true,
        ...
        this.updateResponses = async (req, res, next) => {
            try {
                const { id } = req.params;
                const { responses } = req.body;
                // Check if assessment exists
                const assessmentCheck = await connection_1.db.query('SELECT id, status FROM maturity_assessments WHERE id = $1', [id]);
                ...
                for (const response of responses) {
                    if (response.id) {
                        await connection_1.db.query(`UPDATE maturity_responses
             SET score = $1, evidence = $2, gap_identified = $3, recommendations = $4
             WHERE id = $5 AND assessment_id = $6`, [response.score, response.evidence, response.gapIdentified, response.recommendations, response.id, id]);
                    }
                }
                // Calculate overall score
                const scoreResult = await connection_1.db.query(`SELECT AVG(score) as avg_score FROM maturity_responses
         WHERE assessment_id = $1 AND score IS NOT NULL`, [id]);
                ...
                await connection_1.db.query('UPDATE maturity_assessments SET overall_score = $1 WHERE id = $2', [overallScore, id]);
                res.json({
                    success: true,
        ...
        this.submit = async (req, res, next) => {
            try {
                const { id } = req.params;
                // Check completeness
                const incompleteResult = await connection_1.db.query(`SELECT COUNT(*) as incomplete FROM maturity_responses
         WHERE assessment_id = $1 AND score IS NULL`, [id]);
                ...
                await connection_1.db.query(`UPDATE maturity_assessments SET status = 'Completed' WHERE id = $1`, [id]);
                res.json({
                    success: true,
        ...
        this.getDimensionSummary = async (req, res, next) => {
            try {
                const { id } = req.params;
                const result = await connection_1.db.query(`SELECT dimension, AVG(score) as avg_score, COUNT(*) as question_count,
        ...
        this.delete = async (req, res, next) => {
            try {
                ...
                const check = await connection_1.db.query('SELECT status FROM maturity_assessments WHERE id = $1', [id]);
                ...
                await connection_1.db.query('DELETE FROM maturity_assessments WHERE id = $1', [id]);
                res.json({
                    success: true,
...
exports.MaturityController = MaturityController;
//# sourceMappingURL=maturity.controller.js.map