...
class ScheduledReportController {
    constructor() {
        ...
        this.getAll = async (req, res, next) => {
            try {
                const result = await connection_1.db.query(`SELECT sr.*, u.name as creator_name
        ...
        this.create = async (req, res, next) => {
            try {
                const { name, reportType, format, filters, scheduleType, scheduleDay, scheduleTime, recipients, isActive = true, } = req.body;
                ...
                const result = await connection_1.db.query(`INSERT INTO scheduled_reports
        ...
        this.update = async (req, res, next) => {
            try {
                const { id } = req.params;
                const { name, reportType, format, filters, scheduleType, scheduleDay, scheduleTime, recipients, isActive, } = req.body;
                // Check if exists
                const existing = await connection_1.db.query('SELECT * FROM scheduled_reports WHERE id = $1', [id]);
                ...
                const result = await connection_1.db.query(`UPDATE scheduled_reports
        ...
        this.delete = async (req, res, next) => {
            try {
                const { id } = req.params;
                const result = await connection_1.db.query('DELETE FROM scheduled_reports WHERE id = $1 RETURNING id', [id]);
        ...
        this.toggleActive = async (req, res, next) => {
            try {
                ...
                const nextRunAt = newIsActive
                    ...
                    : null;
                await connection_1.db.query('UPDATE scheduled_reports SET is_active = $1, next_run_at = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3', [newIsActive, nextRunAt, id]);
                res.json({
                    success: true,
        ...
        this.runNow = async (req, res, next) => {
            try {
                const { id } = req.params;
                const existing = await connection_1.db.query('SELECT * FROM scheduled_reports WHERE id = $1', [id]);
                if (existing.rows.length === 0) {
                ...
                const generatedResult = await connection_1.db.query(`INSERT INTO generated_reports
         ...
         RETURNING *`, [
                ...
                await connection_1.db.query('UPDATE scheduled_reports SET last_run_at = CURRENT_TIMESTAMP WHERE id = $1', [id]);
                res.json({
                    success: true,
      ...
      `;
        ...
        this.generateReport = async (req, res, next) => {
            try {
                ...
                // Create a generated report entry
                const result = await connection_1.db.query(`INSERT INTO generated_reports
        ...
        this.deleteGenerated = async (req, res, next) => {
            try {
                const { id } = req.params;
                // Get file path first to delete the file
                const existing = await connection_1.db.query('SELECT file_path FROM generated_reports WHERE id = $1', [id]);
                if (existing.rows.length > 0 && existing.rows[0].file_path) {
                    try {
                        fs_1.default.unlinkSync(existing.rows[0].file_path);
                    }
                    catch (e) {
                        // File may not exist, ignore
                    }
                }
                const result = await connection_1.db.query('DELETE FROM generated_reports WHERE id = $1 RETURNING id', [id]);
                if (result.rowCount === 0) {
