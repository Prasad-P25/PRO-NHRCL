...
async function seed() {
    ...
    `);
        logger_1.logger.info('Roles seeded');
        // Seed Default Project (Sample)
        const projectResult = await connection_1.db.query(`
    ...
    `);
        ...
        logger_1.logger.info(`Default project seeded with ID: ${defaultProjectId}`);
    ...
    `, [defaultProjectId]);
        logger_1.logger.info('Packages seeded');
        // Seed Admin User
    ...
    `, [defaultProjectId]);
        logger_1.logger.info('User project assignments seeded');
    ...
    `);
        logger_1.logger.info('Audit Categories seeded');
        // Seed sample sections and items for Category 1
        const cat1Result = await connection_1.db.query("SELECT id FROM audit_categories WHERE code = '01'");
      ...
      `, [cat1Id]);
            ...
            const sectionAResult = await connection_1.db.query("SELECT id FROM audit_sections WHERE category_id = $1 AND code = 'A'", [cat1Id]);
        ...
        `, [sectionAResult.rows[0].id]);
            ...
            const sectionBResult = await connection_1.db.query("SELECT id FROM audit_sections WHERE category_id = $1 AND code = 'B'", [cat1Id]);
        ...
        `, [sectionBResult.rows[0].id]);
            ...
            const sectionCResult = await connection_1.db.query("SELECT id FROM audit_sections WHERE category_id = $1 AND code = 'C'", [cat1Id]);
        ...
        `, [sectionCResult.rows[0].id]);
            ...
            const sectionDResult = await connection_1.db.query("SELECT id FROM audit_sections WHERE category_id = $1 AND code = 'D'", [cat1Id]);
        ...
        `, [sectionDResult.rows[0].id]);
            ...
            const sectionEResult = await connection_1.db.query("SELECT id FROM audit_sections WHERE category_id = $1 AND code = 'E'", [cat1Id]);
        ...
        `, [sectionEResult.rows[0].id]);
            ...
            const sectionFResult = await connection_1.db.query("SELECT id FROM audit_sections WHERE category_id = $1 AND code = 'F'", [cat1Id]);
        ...
        `, [sectionFResult.rows[0].id]);
            ...
            const sectionGResult = await connection_1.db.query("SELECT id FROM audit_sections WHERE category_id = $1 AND code = 'G'", [cat1Id]);
        ...
        logger_1.logger.info('Audit Sections and Items seeded');
        // Seed KPI Indicators
        await connection_1.db.query(`
    ...
    `);
        logger_1.logger.info('KPI Indicators seeded');
        // Seed sections and items for Category 02 - SHE Management System
        const cat2Result = await connection_1.db.query("SELECT id FROM audit_categories WHERE code = '02'");
        ...
        ON CONFLICT (category_id, code) DO NOTHING
      `, [cat2Id]);
            const sec2AResult = await connection_1.db.query("SELECT id FROM audit_sections WHERE category_id = $1 AND code = 'A'", [cat2Id]);
        ...
        logger_1.logger.info('Category 02 sections and items seeded');
        // Seed sections and items for Category 03 - HIRA
        const cat3Result = await connection_1.db.query("SELECT id FROM audit_categories WHERE code = '03'");
        ...
        ON CONFLICT (category_id, code) DO NOTHING
      `, [cat3Id]);
            const sec3AResult = await connection_1.db.query("SELECT id FROM audit_sections WHERE category_id = $1 AND code = 'A'", [cat3Id]);
        ...
        logger_1.logger.info('Category 03 sections and items seeded');
        // Seed sections and items for Category 04 - Work Permits & LOTO
        const cat4Result = await connection_1.db.query("SELECT id FROM audit_categories WHERE code = '04'");
        ...
        ON CONFLICT (category_id, code) DO NOTHING
      `, [cat4Id]);
            const sec4AResult = await connection_1.db.query("SELECT id FROM audit_sections WHERE category_id = $1 AND code = 'A'", [cat4Id]);
        ...
        `, [sec4AResult.rows[0].id]);
            ...
            const sec4BResult = await connection_1.db.query("SELECT id FROM audit_sections WHERE category_id = $1 AND code = 'B'", [cat4Id]);
        ...
        logger_1.logger.info('Category 04 sections and items seeded');
        // Seed sections and items for Category 05 - Scaffolding & Work at Height
        const cat5Result = await connection_1.db.query("SELECT id FROM audit_categories WHERE code = '05'");
        ...
        ON CONFLICT (category_id, code) DO NOTHING
      `, [cat5Id]);
            const sec5AResult = await connection_1.db.query("SELECT id FROM audit_sections WHERE category_id = $1 AND code = 'A'", [cat5Id]);
        ...
        `, [sec5AResult.rows[0].id]);
            ...
            const sec5BResult = await connection_1.db.query("SELECT id FROM audit_sections WHERE category_id = $1 AND code = 'B'", [cat5Id]);
        ...
        logger_1.logger.info('Category 05 sections and items seeded');
        // Seed sections and items for Category 06 - Excavation
        const cat6Result = await connection_1.db.query("SELECT id FROM audit_categories WHERE code = '06'");
        ...
        ON CONFLICT (category_id, code) DO NOTHING
      `, [cat6Id]);
            const sec6AResult = await connection_1.db.query("SELECT id FROM audit_sections WHERE category_id = $1 AND code = 'A'", [cat6Id]);
        ...
        logger_1.logger.info('Category 06 sections and items seeded');
        // Seed sections and items for Category 08 - Lifting & Cranes
        const cat8Result = await connection_1.db.query("SELECT id FROM audit_categories WHERE code = '08'");
        ...
        ON CONFLICT (category_id, code) DO NOTHING
      `, [cat8Id]);
            const sec8AResult = await connection_1.db.query("SELECT id FROM audit_sections WHERE category_id = $1 AND code = 'A'", [cat8Id]);
        ...
        logger_1.logger.info('Category 08 sections and items seeded');
        // Seed sections and items for Category 09 - Electrical Safety
        const cat9Result = await connection_1.db.query("SELECT id FROM audit_categories WHERE code = '09'");
        ...
        ON CONFLICT (category_id, code) DO NOTHING
      `, [cat9Id]);
            const sec9AResult = await connection_1.db.query("SELECT id FROM audit_sections WHERE category_id = $1 AND code = 'A'", [cat9Id]);
        ...
        logger_1.logger.info('Category 09 sections and items seeded');
        // Seed sections and items for Category 10 - Fire & Emergency
        const cat10Result = await connection_1.db.query("SELECT id FROM audit_categories WHERE code = '10'");
        ...
        ON CONFLICT (category_id, code) DO NOTHING
      `, [cat10Id]);
            const sec10AResult = await connection_1.db.query("SELECT id FROM audit_sections WHERE category_id = $1 AND code = 'A'", [cat10Id]);
        ...
        `, [sec10AResult.rows[0].id]);
            ...
            const sec10BResult = await connection_1.db.query("SELECT id FROM audit_sections WHERE category_id = $1 AND code = 'B'", [cat10Id]);
        ...
        `, [sec10BResult.rows[0].id]);
            ...
            const sec10CResult = await connection_1.db.query("SELECT id FROM audit_sections WHERE category_id = $1 AND code = 'C'", [cat10Id]);
        ...
        logger_1.logger.info('Category 10 sections and items seeded');
        // Seed Sample Audits
        logger_1.logger.info('Seeding sample audits...');
        // Get user and package IDs for creating audits
        const auditorResult = await connection_1.db.query("SELECT id FROM users WHERE email = 'auditor1@protecther.com'");
        const reviewerResult = await connection_1.db.query("SELECT id FROM users WHERE email = 'manager.c2@protecther.com'");
        const package2Result = await connection_1.db.query("SELECT id FROM packages WHERE code = 'C2'");
        const package3Result = await connection_1.db.query("SELECT id FROM packages WHERE code = 'C3'");
      ...
      `, [pkg2Id, auditorId, reviewerId, pkg3Id]);
            ...
            const audit1Result = await connection_1.db.query("SELECT id FROM audits WHERE audit_number = 'AUD-2024-001'");
            const audit4Result = await connection_1.db.query("SELECT id FROM audits WHERE audit_number = 'AUD-2024-004'");
        ...
        `, [audit1Id]);
                ...
                const itemsResult = await connection_1.db.query(`
        ...
        logger_1.logger.info('Sample audits seeded');
        // Seed Sample CAPAs
        logger_1.logger.info('Seeding sample CAPAs...');
        const ncResponsesResult = await connection_1.db.query(`
    ...
    `);
      ...
      `, [
        ...
        logger_1.logger.info('Sample CAPAs seeded');
        // Seed Sample KPI Entries
        logger_1.logger.info('Seeding sample KPI entries...');
        const indicatorsResult = await connection_1.db.query("SELECT id, name FROM kpi_indicators LIMIT 7");
        const packagesResult = await connection_1.db.query("SELECT id FROM packages LIMIT 3");
        ...
        logger_1.logger.info('Sample KPI entries seeded');
