...
async function migrate() {
    ...
    try {
        ...
        logger_1.logger.info('Starting migration: 002-add-projects');
    ...
    `);
        if (tableCheck.rows[0].exists) {
            logger_1.logger.info('Projects table already exists, skipping migration');
            await client.query('COMMIT');
            process.exit(0);
        }
        // 1. Create projects table
        logger_1.logger.info('Creating projects table...');
        await client.query(`
    ...
    `);
        ...
        logger_1.logger.info('Creating user_project_assignments table...');
        await client.query(`
    ...
    `);
        ...
        logger_1.logger.info('Creating default MAHSR project...');
    ...
    `);
        ...
        logger_1.logger.info(`Default project created with ID: ${defaultProjectId}`);
        // 4. Add project_id column to packages (nullable first)
        logger_1.logger.info('Adding project_id column to packages...');
        await client.query(`
    ...
    `);
        ...
        logger_1.logger.info('Linking existing packages to default project...');
        await client.query(`
      UPDATE packages SET project_id = $1
    `, [defaultProjectId]);
        // 6. Make project_id NOT NULL
        logger_1.logger.info('Making project_id NOT NULL...');
        await client.query(`
      ALTER TABLE packages ALTER COLUMN project_id SET NOT NULL
    `);
        // 7. Drop unique constraint on code, add composite unique
        logger_1.logger.info('Updating packages unique constraint...');
        await client.query(`
    ...
    `);
        ...
        logger_1.logger.info('Assigning existing users to default project...');
        await client.query(`
      INSERT INTO user_project_assignments (user_id, project_id, is_default)
      SELECT id, $1, true FROM users
    `, [defaultProjectId]);
        // 9. Create indexes
        logger_1.logger.info('Creating indexes...');
        await client.query(`
    ...
    `);
        await client.query('COMMIT');
        logger_1.logger.info('Migration 002-add-projects completed successfully!');
        process.exit(0);
    }
    catch (error) {
...
migrate();
//# sourceMappingURL=002-add-projects.js.map