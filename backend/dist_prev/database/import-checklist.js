...
async function importChecklist() {
    try {
        ...
        logger_1.logger.info('Reading Excel file: ' + filePath);
        ...
        for (const sheetName of wb.SheetNames) {
            ...
            if (!mapping) {
                logger_1.logger.info('Skipping sheet: ' + sheetName);
                continue;
            }
            ...
            logger_1.logger.info(`  Found ${sections.length} sections with ${sections.reduce((sum, s) => sum + s.items.length, 0)} items`);
        }
        // Now insert into database
        logger_1.logger.info('Inserting data into database...');
        for (const category of categories) {
            // Check if category exists
            let catResult = await connection_1.db.query(`SELECT id FROM audit_categories WHERE code = $1`, [category.code]);
            let categoryId;
            if (catResult.rows.length > 0) {
                // Update existing
                categoryId = catResult.rows[0].id;
                await connection_1.db.query(`UPDATE audit_categories SET name = $1, full_title = $2 WHERE id = $3`, [category.name, category.fullTitle, categoryId]);
            }
            else {
                // Insert new
                catResult = await connection_1.db.query(`INSERT INTO audit_categories (code, name, full_title, type, display_order, is_active)
           VALUES ($1, $2, $3, 'Compliance', $4, true)
           RETURNING id`, [category.code, category.name, category.fullTitle, parseInt(category.code)]);
                categoryId = catResult.rows[0].id;
            }
            // Delete existing sections and items for this category (clean import)
            await connection_1.db.query(`DELETE FROM audit_items WHERE section_id IN (SELECT id FROM audit_sections WHERE category_id = $1)`, [categoryId]);
            await connection_1.db.query(`DELETE FROM audit_sections WHERE category_id = $1`, [categoryId]);
            ...
            for (let secIndex = 0; secIndex < category.sections.length; secIndex++) {
                ...
                const secResult = await connection_1.db.query(`INSERT INTO audit_sections (category_id, code, name, display_order)
           VALUES ($1, $2, $3, $4)
           RETURNING id`, [categoryId, section.code, section.name, secIndex + 1]);
                const sectionId = secResult.rows[0].id;
                // Insert items
                for (const item of section.items) {
                    await connection_1.db.query(`INSERT INTO audit_items (section_id, sr_no, audit_point, standard_reference, evidence_required, priority, is_active)
             VALUES ($1, $2, $3, $4, $5, $6, true)`, [sectionId, item.srNo, item.auditPoint, item.standardReference, item.evidenceRequired, item.priority || 'P1']);
                }
            }
            logger_1.logger.info(`  Imported category ${category.code}: ${category.name}`);
        }
        // Count total items
        const countResult = await connection_1.db.query(`SELECT COUNT(*) as count FROM audit_items`);
        logger_1.logger.info(`Total audit items in database: ${countResult.rows[0].count}`);
        logger_1.logger.info('Import completed successfully!');
        process.exit(0);
    }
    catch (error) {
...
importChecklist();
//# sourceMappingURL=import-checklist.js.map